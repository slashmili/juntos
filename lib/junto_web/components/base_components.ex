defmodule JuntoWeb.BaseComponents do
  @moduledoc """
  Provides base UI components.


  No class or css framework should be used in this module
  """
  ...
  def base_modal(assigns) do
    ~H"""
    <div
      id={@id}
      phx-mounted={@show && base_show_modal(@id)}
      phx-remove={base_hide_modal(@id)}
      data-cancel={JS.exec(@on_cancel, "phx-remove")}
      class="relative z-50 hidden"
    >
      <div
        id={"#{@id}-bg"}
        class="bg-zinc-50/20 dark:bg-zinc-950/20 fixed inset-0 transition-opacity"
        aria-hidden="true"
      />
      <div
  ...
  def base_flash(assigns) do
    ...
    <div
      ...
      id={@id}
      phx-click={JS.push("lv:clear-flash", value: %{key: @kind}) |> base_hide("##{@id}")}
      role="alert"
      class={[
  ...
  def base_flash_group(assigns) do
    ~H"""
    <div id={@id}>
      <.base_flash kind={:info} title={gettext("Success!")} flash={@flash} />
      <.base_flash kind={:error} title={gettext("Error!")} flash={@flash} />
      <.base_flash
        id="client-error"
        kind={:error}
  ...
  def base_simple_form(assigns) do
    ~H"""
  ...
  def base_button(assigns) do
    ~H"""
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 phx-submit-loading:btn-outline ",
        "btn btn-primary",
        @class
      ]}
      {@rest}
  ...
  def base_input(%{type: "checkbox"} = assigns) do
    assigns =
  ...
  ## Examples
      ...
      <.base_icon name="hero-x-mark-solid" />
      <.base_icon name="hero-arrow-path" class="ml-1 w-3 h-3 animate-spin" />
  """
  attr :name, :string, required: true
  attr :class, :string, default: nil

  def base_icon(%{name: "hero-" <> _} = assigns) do
    ~H"""
  ...
  def base_hide(js \\ %JS{}, selector) do
    JS.hide(js,
      to: selector,
      time: 200,
      transition:
  ...
  def base_show_modal(js \\ %JS{}, id) when is_binary(id) do
    js
    |> JS.show(to: "##{id}")
    |> JS.show(
      to: "##{id}-bg",
  ...
  def base_hide_modal(js \\ %JS{}, id) do
    js
    |> JS.hide(
      to: "##{id}-bg",
      transition: {"transition-all transform ease-in duration-200", "opacity-100", "opacity-0"}
    )
    |> base_hide("##{id}-container")
